...
asynchronous event handler, add `async=True` argument to
py:func:`qubes.events.handler` decorator. Such handler must be a coroutine (for
example with py:func:`asyncio.coroutine` decorator). By definition, order of
...
handler (with `async=True`) for synchronous event (the one fired with
...
.. code-block:: python
   ...
   class MyClass(qubes.events.Emitter):
       @qubes.events.handler('event1', 'event2', async=True)
       @asyncio.coroutine
       def event_handler(self, event):
           if event == 'event1':
...
.. code-block:: python
   ...
   class MyClass(qubes.events.Emitter):
       @qubes.events.handler('event1', async=True)
       @asyncio.coroutine
       def event_handler(self, event):
