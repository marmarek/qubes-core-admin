...
class TC_00_ThinPool(ThinPoolBase):
    ...
    def test_009_interrupted_commit(self):
        ...
        # interrupted commit
        revisions = ['-1.1521065904', '-2.1521065905', '-snap']
        orig_uuids = {}
    ...
    def test_010_migration1(self):
        ...
        with unittest.mock.patch('time.time') as mock_time:
            ...
            volume.stop()
        revisions.extend(['-1.1521065906', '-2.1521065907'])
        expected_revisions = {
            revisions[2].lstrip('-'): '2018-03-14T22:18:25',
            revisions[3].lstrip('-'): '2018-03-14T22:18:26',
        }
        self.assertEqual(volume.revisions, expected_revisions)
        self.assertEqual(volume.path, '/dev/' + volume.vid + revisions[4])
        path_snap = '/dev/' + volume._vid_snap
        self.assertFalse(os.path.exists(path_snap), path_snap)
        self.assertFalse(os.path.exists('/dev/' + volume.vid))
    ...
    def test_011_migration2(self):
        ...
        self.assertEqual(volume.revisions, expected_revisions)
        self.assertEqual(volume.path, '/dev/' + volume.vid + revisions[3])
        path_snap = '/dev/' + volume._vid_snap
        self.assertFalse(os.path.exists(path_snap), path_snap)
        self.assertFalse(os.path.exists('/dev/' + volume.vid))
    ...
    def test_013_migration4(self):
        ...
        expected_revisions = {}
        self.assertEqual(volume.revisions, expected_revisions)
        self.assertEqual(volume.path, '/dev/' + volume.vid + '-2.1521065907')
    ...
    def test_020_revert_last(self):
        ...
        path_snap = '/dev/' + volume._vid_snap
        self.assertFalse(os.path.exists(path_snap), path_snap)
        self.assertNotEqual(current_path, volume.path)
    ...
    def test_032_import_volume_same_pool(self):
        ...
        expected_revisions = {
            '1.1521065905': '2018-03-14T22:18:25',
        }
        self.assertEqual(volume.revisions, expected_revisions)
    ...
    def test_100_pool_list_volumes(self):
        ...
        expected_list = [volume1, volume2]
        self.assertCountEqual(list(self.pool.volumes), expected_list)
        volume1.remove()
        self.assertCountEqual(list(self.pool.volumes), [volume2])
        volume2.remove()
        self.assertCountEqual(list(self.pool.volumes), [])
