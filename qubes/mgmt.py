...
class QubesMgmt(object):
    ...
    def pool_info(self, untrusted_payload):
        ...
        return ''.join('{}={}\n'.format(prop, val)
            for prop, val in pool.config.items())
    ...
    def pool_add(self, untrusted_payload):
        ...
        del untrusted_payload
        # final '\n' in input cause the last line being empty
        assert not untrusted_pool_config[-1]
        untrusted_pool_config = untrusted_pool_config[:-1]
